...
def get_cfgs():
    env_cfg = {
        ...
        # --- CRITICAL CHANGES ---
        "kp": 45.0,                # Increased from 20: Need stiffness for high speed
        "kd": 1.5,                 # Increased from 0.5: Prevents jitter at high Kp
        "termination_if_roll_greater_than": 45,  # Loosened to allow leaning
    ...
    obs_cfg = {
        "num_obs": 45,
        "obs_scales": {
            "lin_vel": 1.0,        # Reduced from 2.0: Keeps NN inputs closer to [-1, 1]
            "ang_vel": 0.25,
            "dof_pos": 1.0,
            "dof_vel": 0.05,
        },
    ...
    reward_cfg = {
        ...
        "reward_scales": {
            "tracking_lin_vel": 12.0,      # Pushes the robot to move
            "tracking_ang_vel": 2.0,
            "lin_vel_z": -2.0,
            "ang_vel_xy": -0.1,
            "action_rate": -0.001,        # Minimal penalty to allow fast movement
    ...
    command_cfg = {
        "num_commands": 3,
        # --- THE CURRICULUM ---
        # Start at 0.0 so it learns to stand/walk before it learns to sprint!
